...
const Body = ({data, setRetry, tasks}) => {
	...
	const taskSteps = useMemo(() => {
		...
		tasks.forEach(task => {
			if (
				...
				const step = {};
				step[task.name] = [task];
				versions[task.workflowDefinitionVersion] = step;
			}
		});
	...
	const versionedCards = useMemo(
		() =>
			Object.entries(taskSteps)
				.flatMap(array => array[1])
				.map(version => Object.entries(version)),
		[taskSteps]
	);
	...
	const nextTransitions = useMemo(() => {
		...
		workflowTaskTransitions.forEach(
			({transitions, workflowDefinitionVersion, workflowTaskName}) => {
				if (!taskTransitions[workflowDefinitionVersion]) {
					const step = {};
					step[workflowTaskName] = transitions;
					taskTransitions[workflowDefinitionVersion] = step;
				}
		...
		return Object.entries(taskTransitions).flatMap(array => array[1]);
