...
import SLAFormPage from '../../../../src/main/resources/META-INF/resources/js/components/sla/sla-form-page/SLAFormPage.es';
...
describe('The SLAFormPage component should', () => {
	...
	describe('Create a new SLA', () => {
		...
		test('Redirect to SLAListPage after succesfull submit', async () => {
	...
	describe('Edit a SLA', () => {
		...
		beforeAll(() => {
			...
			renderResult = render(
				<MockRouter client={clientMock}>
					<ToasterProvider>
						<SLAFormPage history={historyMock} id="1234" processId="5678" />
					</ToasterProvider>
				</MockRouter>
			);
	...
	describe('Edit a Blocked SLA', () => {
		...
		test('Handle errors at start and stop node keys', () => {
			const alertWorkflowDefinitionChange = getByTestId(
				'alertWorkflowDefinitionChange'
			);
			const startField = getByTestId('startField');
			const stopField = getByTestId('stopField');

			expect(alertWorkflowDefinitionChange).toHaveTextContent(
