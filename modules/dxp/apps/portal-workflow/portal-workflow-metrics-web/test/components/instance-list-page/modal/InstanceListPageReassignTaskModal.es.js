...
import InstanceListPage, {
	ReassignTaskModalContext
} from '../../../../src/main/resources/META-INF/resources/js/components/instance-list-page/InstanceListPage.es';
...
describe('The ReassignTaskModal component should', () => {
	afterEach(() => cleanup);
		...
		const {getByTestId} = render(
			<MockRouter clientHeadless={clientMock}>
				...
				>
					...
					>
						<InstanceListPage.ReassignTaskModal />
					</ReassignTaskModalContext.Provider>
				</InstanceListContext.Provider>
			</MockRouter>
		);
	...
	test('Render LoadingView component', () => {
		const {getAllByTestId} = render(
			<ReassignTaskModal.LoadingView></ReassignTaskModal.LoadingView>
		);
	...
	test('Render ErrorView component', () => {
		const {getByTestId} = render(
			<ReassignTaskModal.ErrorView></ReassignTaskModal.ErrorView>
		);
	...
	test('Fire onClose by clicking the Cancel button', () => {
		cleanup();
		const {getAllByTestId} = render(
			<MockRouter clientHeadless={clientMock}>
				<InstanceListContext.Provider
