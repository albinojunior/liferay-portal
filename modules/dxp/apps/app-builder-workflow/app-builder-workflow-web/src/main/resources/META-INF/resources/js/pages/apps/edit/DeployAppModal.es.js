...
export default ({appId, objectId, onCancel}) => {
	const {
		isDeployAppVisible,
		setDeployAppVisible,
		state: {app},
	...
	const getStandaloneLink = (appId) => {
		...
		return (
			<ClayLink href={getStandaloneURL(appId)} target="_blank">
	...
	const onClickDeploy = () => {
		if (appId) {
			...
			addItem(
				`/o/app-builder/v1.0/data-definitions/${objectId}/apps`,
				app
			)
				.then(() => onSuccess(app.id))
				.then(onCancel)
				.catch(onError);
		}
	};
